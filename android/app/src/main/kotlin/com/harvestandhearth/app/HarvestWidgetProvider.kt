...
class HarvestWidgetProvider : HomeWidgetProvider() {
    override fun onReceive(context: Context, intent: Intent) {
        try {
            ...
            // Defensive guard: keep provider process alive on launcher-specific edge cases.
        }
    }
        ...
        appWidgetIds.forEach { widgetId ->
            try {
                val views = RemoteViews(context.packageName, R.layout.harvest_widget).apply {
                    ...
                    if (expiredCount > 0) {
                        setInt(R.id.widget_root, "setBackgroundResource", R.drawable.widget_bg_danger)
                        setInt(R.id.widget_status_chip, "setBackgroundResource", R.drawable.widget_status_danger)
                        setTextColor(R.id.widget_status_chip, Color.parseColor("#B71C1C"))
                    } else if (expiringCountNum > 0) {
                        setInt(R.id.widget_root, "setBackgroundResource", R.drawable.widget_bg_warning)
                        setInt(R.id.widget_status_chip, "setBackgroundResource", R.drawable.widget_status_warning)
                        setTextColor(R.id.widget_status_chip, Color.parseColor("#E65100"))
                    } else {
                        setInt(R.id.widget_root, "setBackgroundResource", R.drawable.widget_bg_safe)
                        setInt(R.id.widget_status_chip, "setBackgroundResource", R.drawable.widget_status_safe)
                    ...
                    setViewVisibility(R.id.widget_updated_at, if (updatedAt.isBlank()) View.GONE else View.VISIBLE)
                    ...
                    if (expiringCount == null && line1.isNotBlank()) {
                        ...
                        setTextViewText(R.id.widget_expiring_value, expiringCount ?: "0")
                        setTextViewText(
                        ...
                        setTextViewText(R.id.widget_expired_value, expiredCountRaw)
                    }

                    val line2 = widgetData.getString("line2", "") ?: ""
                    setTextViewText(R.id.widget_line2, line2)
                    setViewVisibility(R.id.widget_line2, if (line2.isBlank()) View.GONE else View.VISIBLE)
                ...
                val fallback = RemoteViews(context.packageName, R.layout.harvest_widget_minimal).apply {
                    setTextViewText(R.id.widget_title, "Harvest & Hearth")
